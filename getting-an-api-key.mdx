...
```bash
curl -X POST "https://open-energie.api.vwala.be/auth/otp/send" \
...
```bash
curl -X POST "https://open-energie.api.vwala.be/auth/otp/verify" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "https://open-energie.api.vwala.be/api-keys" \
  -H "Content-Type: application/json" \
...
curl -H "x-api-key: YOUR_API_KEY" \
  "https://open-energie.api.vwala.be/v1/merchants"
```
...
curl -H "Authorization: Bearer YOUR_SESSION_JWT" \
  "https://open-energie.api.vwala.be/api-keys"
```
...
```bash
curl -X DELETE "https://open-energie.api.vwala.be/api-keys/KEY_UUID" \
  -H "Authorization: Bearer YOUR_SESSION_JWT"
```
