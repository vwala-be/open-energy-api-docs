...
**List your keys** (replace `YOUR_SESSION_JWT` with a fresh JWT from the OTP flow):
...
**Revoke a key** (replace `YOUR_SESSION_JWT` and `KEY_UUID` with the key’s `uuid` from the list):
